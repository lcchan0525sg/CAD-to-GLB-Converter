...
1. Review `git status --short` and `git diff`, including the README and MIT license.
...
For a Draco-enabled package, include the upstream Draco license and notices with its runtime. The repository includes the Draco 1.5.7 license. Review the notices supplied with the exact OCCT and Draco distributions used for the release.
